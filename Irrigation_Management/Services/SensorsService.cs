...
namespace Irrigation_Management.Services
{
    ...
    public class SensorsService : ISensorsService
    {
        ...
        {
            return await _sensorsRepository.DeleteSensor(Sensor_Id);
        }
        ...
        public async Task<Sensors?> UpdateSensor(int Sensor_Id, int Sensor_Type_Id = -1, int Device_Id = -1)
        {
            ...
            {
                if (Sensor_Type_Id != -1) sensorToUpdate.Sensor_Type_Id = Sensor_Type_Id;
                if (Device_Id != -1) sensorToUpdate.Device_Id = Device_Id;
