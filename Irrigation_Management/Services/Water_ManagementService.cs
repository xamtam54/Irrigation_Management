...
namespace Irrigation_Management.Services
{
    ...
    public class WaterManagementService : IWaterManagementService
    {
        ...
        public WaterManagementService(IWaterManagementRepository waterManagementRepository)
        {
        ...
        public async Task<Water_Management?> UpdateWaterManagement(int Water_Management_Id, float? Capacity = -1, float? Collection_Hour = null, int? Device_Id = -1, int? Water_Management_Type_Id = -1)
        {
            ...
            {
                if (Capacity != -1){waterManagementToUpdate.Capacity = (float)Capacity;}
                if (Collection_Hour != null){waterManagementToUpdate.Collection_Hour = Collection_Hour;}
                if (Device_Id != -1){waterManagementToUpdate.Device_Id = (int)Device_Id;}
                if (Water_Management_Type_Id != -1){waterManagementToUpdate.Water_Management_Type_Id = (int)Water_Management_Type_Id;}
        ...
        {
            Water_Management? management = await _waterManagementRepository.GetWaterManagement(Water_Management_Id);

            if (management != null)
            {
                management.IsDeleted = true;
                // management.Date = DateTime.Now;
                return await _waterManagementRepository.DeleteWaterManagement(management);
            }
